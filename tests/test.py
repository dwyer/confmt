...
with open(CONFIG_FILENAME) as f:
    ...
    assert env[''] == ''
    compare(baseenv, env)
    content = sorted(simpleconf.dumps(env).splitlines())

# test object_type
with open(CONFIG_FILENAME) as f:
    env = simpleconf.load(f, object_type=collections.OrderedDict)
    assert isinstance(env, collections.OrderedDict)
    compare(baseenv, env)
    compare(content, sorted(simpleconf.dumps(env).splitlines()))

# test parse_int/float
with open(CONFIG_FILENAME) as f:
    env = simpleconf.load(f, parse_int=str, parse_float=str)
...
# test parse_*
with open(CONFIG_FILENAME) as f:
    env = simpleconf.load(f, parse_int=float,
                          parse_float=lambda x: int(float(x)))
...
with open(CONFIG_FILENAME) as f:
    env = simpleconf.load(f, keywords={})
    tempenv = baseenv.copy()
    tempenv['yes'] = 'true'
    tempenv['no'] = 'false'
    tempenv['none'] = 'null'
...
with open(CONFIG_FILENAME) as f:
    s = f.read().replace('#', ';')
    env = simpleconf.loads(s, comment_token=';')
    compare(env, baseenv)
    compare(content, sorted(simpleconf.dumps(env).splitlines()))
...
with open(CONFIG_FILENAME) as f:
    ...
    env = simpleconf.loads(s, assignment_operator=':')
    compare(env, baseenv)
    compare(content, sorted(simpleconf.dumps(env).splitlines()))
...
with open(CONFIG_FILENAME) as f:
    env = simpleconf.load(
        f, key_validator=lambda s: re.match(r'[\w_][\w\d_]*', s))
    tempenv = baseenv.copy()
    del tempenv['']
    compare(env, tempenv)
    compare(content[1:], sorted(simpleconf.dumps(env).splitlines()))

    def valid(s):
        return ' ' not in s

    env = simpleconf.loads('a.b = 1 \n a.b.c d = 2', key_separator='.',
                           key_validator=valid)
    assert env['a']['b'] == 1
    env = simpleconf.loads('a.b c.d = 1', key_separator='.',
                           key_validator=valid)
    assert not env

# test key_separator
with os.popen('git config --list --local') as f:
    env = simpleconf.load(f, key_separator='.', parse_int=float)
    opts = dict(key_separator='->', keywords={'yes': True, 'no': False})
    s = simpleconf.dumps(env, **opts)
    compare(env, simpleconf.loads(s, **opts))

# test escape_token
s = 'a \= b = \#t # this should be #t'
keywords = {'#t': True, '#f': False}
obj = simpleconf.loads(s, keywords=keywords)
assert 'a = b' in obj and obj['a = b'] is True, obj
t = simpleconf.dumps({'a = b': True}, keywords=keywords)
assert s.startswith(t), (s, t)

obj = simpleconf.loads('a.b = \#t \n a\.b = \#f', keywords=keywords,
                       key_separator='.')
